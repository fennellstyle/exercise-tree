...
// todo: Templatize the printer

// Find the maximum height of the binary tree
int maxHeight(const node_ref_t &p) {
  if (!p) return 0;
...
void printBranches(int branchLen, int nodeSpaceLen, int startLen, int nodesInThisLevel, const deque<node_ref_t>& nodesQueue, ostream& out) {
  deque<node_ref_t>::const_iterator iter = nodesQueue.begin();
...
void printNodes(int branchLen, int nodeSpaceLen, int startLen, int nodesInThisLevel, const deque<node_ref_t>& nodesQueue, ostream& out) {
  deque<node_ref_t>::const_iterator iter = nodesQueue.begin();
...
void printLeaves(int indentSpace, int level, int nodesInThisLevel, const deque<node_ref_t>& nodesQueue, ostream& out) {
  deque<node_ref_t>::const_iterator iter = nodesQueue.begin();
  for (int i = 0; i < nodesInThisLevel; i++, iter++) {
...
void printPretty(const node_ref_t root, int level, int indentSpace, ostream& out) {
